...
> hidup**.
>
> **Soal kualitas sumber (penting):** SIPONGI menerbitkan ulang data NASA
> (VIIRS/MODIS) **dengan tingkat kepercayaan** — tervalidasi. Sedangkan BMKG
> GeoHotspot memakai **Himawari** sebagai saringan **anomali suhu kecerahan**
> tanpa field confidence/FRP (sudah dicek langsung ke layanannya) — banyak
> titik palsu dari lahan panas/perkotaan. Karena itu:
> * **Hanya SIPONGI yang boleh menaikkan status** (aman → waspada). BMKG
>   tampil sebagai baris info + titik oranye di peta, **tidak mengubah status**.
> * Tautan "peta BMKG GeoHotspot" sengaja **dihapus** dari dashboard: peta
>   nasionalnya penuh titik termasuk area tanpa api — menyesatkan. Peta titik
>   BMKG yang sudah terfilter radius kebun tetap ada di `peta.html`.
...
**Peta titik api (`peta.html`).** Halaman peta mandiri (Leaflet + OSM, tanpa
API key) yang memplot kebun, lingkar radius pengawasan, dan **semua titik**
FIRMS (merah) + sumber sekunder (oranye) dari field `points` di
`lingkungan.json` — jawaban langsung "di mana apinya?" tanpa situs eksternal.
Titik BMKG diberi catatan bahwa itu anomali suhu indikatif, bukan kebakaran
tervalidasi.
