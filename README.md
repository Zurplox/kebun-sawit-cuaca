...
`farm_lon`) dan semua titik SIPONGI disaring otomatis berdasar jarak dari kebun —
tak perlu pilih provinsi/tanggal. Tautan unduhan SIPONGI juga ditemukan **otomatis**
dari halaman [Data Hotspot SiPongi+](https://sipongi.gakkum.kehutanan.go.id/sebaran-titik-panas)
(auto-discovery); bila gagal, daftar `sipongi_urls` dicoba sebagai cadangan.

Setelan di `config.json` (semua opsional):

* `use_sipongi` — `true`/`false` untuk mematikan lapis SIPONGI.
* `sipongi_url` — cadangan terakhir saja; biasanya **dibiarkan kosong**.
* `sipongi_urls` — daftar URL cadangan yang dicoba berurutan (KMZ/TXT/JSON didukung).

> Setiap penggunaan data yang bersumber dari SIPONGI wajib mencantumkan: **SIPONGI KEMENHUT**.
