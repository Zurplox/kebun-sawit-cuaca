...
def env_lines(env, ins=None):
    ...
    if st == "tidak tersedia":
        ...
        _src = " (SIPONGI)" if near.get("src") == "SIPONGI" else ""
    ...
    if sp.get("status") and sp.get("status") != "tidak tersedia" and fire.get("primary_src") != "SIPONGI":
        if sp.get("count_within"):
            _sn = sp.get("nearest") or {}
            _sp = " - Cek silang SIPONGI: " + str(sp["count_within"]) + " titik ≤" + str(sp.get("warn_km", 50)) + " km"
            if _sn.get("km") is not None:
                _sp += " · terdekat " + str(_sn["km"]) + " km (" + str(_sn.get("dir", "")) + ")"
            L.append(_sp)
        else:
            L.append(" - Cek silang SIPONGI: nihil di sekitar kebun")
