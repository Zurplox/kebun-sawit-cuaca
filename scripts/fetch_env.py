...
def _fire_points_list(within, cap=300):
    """Daftar titik FIRMS dalam radius -> dipakai peta.html untuk memplot
    SEMUA titik (bukan cuma yang terdekat). Diurutkan dari yang terdekat."""
    out = []
    for km, la, lo, r in within[:cap]:
        out.append({
            "lat": round(la, 4), "lon": round(lo, 4), "km": round(km, 1),
            "acq": (r.get("acq_date") or "").strip() or None,
            "sat": r.get("satellite") or r.get("instrument"),
            "confidence": r.get("confidence"), "frp": r.get("frp"),
        })
    return out
...
def fetch_fire(cfg):
    ...
    if not pts:
        return {"status": "aman", "warn_km": warn_km, "nearest": None,
                "count_within": 0, "scanned_days": days, "points": []}
    ...
    return {"status": status, "warn_km": warn_km, "nearest": nearest,
            ...
            "points": _fire_points_list(within)}
...
def _sp_points_list(within, cap=300):
    """Daftar titik sekunder (SIPONGI/BMKG) dalam radius -> untuk peta.html."""
    out = []
    for km, la, lo, p in within[:cap]:
        out.append({
            "lat": round(la, 4), "lon": round(lo, 4), "km": round(km, 1),
            "conf": p.get("conf"), "date": p.get("date"),
            "name": p.get("name") or "",
        })
    return out
...
def fetch_fire_sipongi(cfg):
    ...
    if used is None:
        return {"status": "tidak tersedia",
                ...
                "warn_km": warn_km, "nearest": None, "count_within": 0,
                "points": [], "total_points": 0, "source_label": None}
    ...
    if not geo:
        return {"status": "aman", "warn_km": warn_km, "nearest": None,
                "count_within": 0, "points": [], "total_points": 0,
...
def combine_fire(fire, sp, cfg):
    ...
    # HANYA sumber TERVALIDASI (SIPONGI: VIIRS/MODIS + tingkat kepercayaan) yang
    # boleh menaikkan status. BMKG GeoHotspot = anomali suhu kecerahan Himawari
    # TANPA confidence/FRP (layanannya tak punya field kualitas -- banyak titik
    # palsu dari lahan panas/perkotaan) -> tampil sebagai baris info SAJA,
    # jangan pernah mengubah status FIRMS supaya tak ada alarm keliru.
    if (tag == "SIPONGI" and fire.get("status") == "aman"
            and sp.get("status") in ("waspada", "bahaya")):
